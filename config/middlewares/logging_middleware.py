...
NO_LOGGING_DEFAULT_VALUE = getattr(
    settings, "DJANGO_REQUEST_LOGGING_NO_LOGGING_DEFAULT_VALUE", False
)
LOG_HEADERS_ATTR = "log_headers"
LOG_HEADERS_DEFAULT_VALUE = getattr(
    settings, "DJANGO_REQUEST_LOGGING_LOG_HEADERS_DEFAULT_VALUE", True
)
...
LOG_BODY_DEFAULT_VALUE = getattr(
    settings, "DJANGO_REQUEST_LOGGING_LOG_BODY_DEFAULT_VALUE", True
)
...
LOG_RESPONSE_DEFAULT_VALUE = getattr(
    settings, "DJANGO_REQUEST_LOGGING_LOG_RESPONSE_DEFAULT_VALUE", True
)
NO_RESPONSE_LOGGING_MSG_ATTR = "no_response_logging_msg"
NO_RESPONSE_LOGGING_MSG = "No response logging for this endpoint"

LOGGER_NAME = getattr(settings, "DJANGO_REQUEST_LOGGING_LOGGER_NAME", "django.request")
...
class LoggingMiddleware(object):
    def __init__(self, get_response=None):
        ...
        self.logger = ColourLogger("green", "red") if enable_colorize else Logger()
    ...
    def _skip_logging_request(self, request, reason):
        method_path = self._method_path(request)
        no_log_context = {
            "args": (),
        ...
        if not log_headers:
            if because is not None:
                self.logger.log_error(
                    logging.INFO,
                    "no headers logged",
                    {"args": {}, "kwargs": {"extra": {"no_header_logging": because}}},
                )
            return None
        ...
        if not log_body:
            if because is not None:
                self.logger.log_error(
                    logging.INFO,
                    "no body logged",
                    {"args": {}, "kwargs": {"extra": {"log_body": because}}},
                )
            return None
        ...
        if not log_response:
            if because is not None:
                self.logger.log_error(
                    logging.INFO,
                    resp_log,
                    {"args": {}, "kwargs": {"extra": {"log_response": because}}},
                )
            return response
